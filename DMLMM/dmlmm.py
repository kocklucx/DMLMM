...
class DMLMM:
    def __init__(self,y,x,K,D):
        ...
        self.L=len(self.K) # Number of layers (assumed to be the length of K)
        # Hyperparmeters
        ...
        self.n=len(self.y) # number of data points (assumed to be the length of y)
        self.D[0]=len(self.x[0][0]) # ensuer that D[0] equals the number of features in the design matrices
        # Initialize several lists and tensors to hold the parameters of the variational approximation
        ...
        init=min(self.n,8192) # Subsample size to reduce computational time on large datasets
        w=self.n/init #weight to correct for subsample
        # Compute a naive estimate for the variational approximation as a starting point
        Z=[]
        eps_loc=[]
        var_starter=[]
        ...
        # Clamp all parameters to avoid overflow errors (safety measure)
    ...
    # This method computes and returns the parameters of the Gaussian Mixture Model corresponding to the posterior mode
    # based on the current estimates for lambda of the DMFAprior
        delta_hat=[] 
        p_hat=[]
        mu_hat=[]
        b_hat=[]
        ...
        mu_gmm_hat=[] # List to store GMM means
        sigma_gmm_hat=[] # List to store GMM covariances
        weight_gmm_hat=[] # List to store GMM weights
        paths=self.all_paths(self.K) # Get all possible paths of clusters across layers
        for path in paths: # loop over all possible path
            weight=1
            for l in range(self.L):
    ...
    # This method takes a list K as an argument. K contains the number of clusters in each layer of the DMFA.
    # It returns a list of all possible paths through the DMFA-prior.
        paths=list()
        anzahl_paths=1
        for k in K:
            anzahl_paths*=k
    ...
    # This method computes the mean (m) and variance (S) for the marginal distribution of z up to a given layer
    # along a specified path in the model. The path is represented as a sequence of cluster assignments across layers.
    ...
    # This method calculates the ELBO as explained in the appendix of the paper and estimates its value using a minibatch indexed by the list of indices "index"  
        batchsize=len(index)
        T=0
    ...
    def train(self,max_iter,threshold):
        # The train method implements a gradient ascent-based optimization algorithm for training a DMFA. 
        # This method is structured to iteratively update the parameters of the model through a series of steps, 
        # incorporating both natural gradients and a gradient ascent strategy for optimization.
        # max_iter is the maximum number of steps.  If the change in ELBO is below the specified threshold 
        # over a certain number of iterations, the method terminates early. 
        batchsize=int(min(max(0.05*self.n,2*self.K[0]),1024)) # number of data points used for each iteration
        ...
        for wdh_global in range(max_iter): 
            ...
            #update local/ auxillary parameters
            with torch.no_grad():
                eps_alpha=self.eps
                alpha=[]
                z_m=[]
                ...
                for i in range(batchsize):
                    ...
                    for l in range(self.L):
                        ...
                        fix_z=z_m[l+1][i]         
            #Estimate the gradient of the ELBO along the minibatch for all global parameters
            elbo=self.calc_elbo(index,z_m,z_sigma,alpha)
            elbo_ts.append(elbo.detach().item())
            elbo.backward()
            # calculate natural gradients and update the model parameters
            with torch.no_grad():
        ...
        self.elbo=np.mean(elbo_ts[-round(len(elbo_ts)*0.05):])
        self.cluster_gmm()
    ...
    def cluster_gmm(self):
        # clusters the data points into GMM components based on the learned parameters. 
        # It generates s_gmm, w_gmm, and m_gmm which are covariance matrices, weights and means of the Gaussian components
        # Weights for components with no data points assigned get set to zero
        ...
        self.m_gmm = m[w>0,:]
        self.w_gmm = w[w>0]
        self.error_var = (self.sigma_b/(self.sigma_a+1)).item()
    ...
    def predict_y(self,design_pred,y_obs,n):
        # calculates the predictive distribution for unseen data points based on the design matrix design_pred. It generates n samples
        # from the predicitive distribution. If n=0, it returns the GMM parameters of the predictive distribution.
        w_pred = []
        m_pred = []
        s_pred = []
